...
loc_exec = "C:\\Users\\Tobias\\Anaconda3\\Lib\\site-packages\\rgf\\lib\\bin\\rgf.exe"

#Location of a temporary directory (has to exist)
loc_temp = "C:\\Users\\Tobias\\Anaconda3\\Lib\\site-packages\\rgf\\lib\\temp3"

class RegularizedGreedyForestClassifier(BaseEstimator, ClassifierMixin):
	def __init__(self, verbose=0, max_leaf=500, test_interval=100, loc_exec=loc_exec, loc_temp=loc_temp, algorithm="RGF", loss="LS", l2=1.0, prefix="model"):
	...
	def predict(self, X, clean=True):
		...
		def binarize(x):
			if x>0:
				...
				return 0
		y_pred = np.array([binarize(x) for x in y_pred])
		#Clean temp directory
		if clean:
			model_glob = self.loc_temp + os.sep + "*"
...
class RegularizedGreedyForestRegressor(BaseEstimator, RegressorMixin):
